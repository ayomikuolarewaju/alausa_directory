...
export async function generateStaticParams() {
  return entities.filter(e => e.category === "ministry").map(e => ({ id: e.id }));
}
...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  const commissioner = entity.officers.find(o => o.title.includes("Commissioner"))?.name ?? entity.officers[0]?.name ?? "";
  const description = `${entity.name} — Lagos State Government. Commissioner: ${commissioner}. Address: ${entity.address}. Contact: ${entity.phone}.`;
  const url = `${BASE_URL}/ministries/${id}`;
  return {
    title,
    description,
...
export default async function DetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const entity = getById(id);
  if (!entity || entity.category !== "ministry") notFound();

  const commissioner = entity.officers.find(o => o.title.includes("Commissioner"));
  ...
  const jsonLd = {
    ...
    url: entity.website ?? `${BASE_URL}/ministries/${entity.id}`,
    address: {
      "@type": "PostalAddress",
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-amber-700 text-xs mt-1">
                  The leadership details for this ministry have not been confirmed by phone. 
        ...
        <div className="rounded-2xl overflow-hidden mb-8" style={{ border: "2px solid #0D0D0D", boxShadow: "6px 6px 0 #0D0D0D" }}>
          <div style={{ background: COLOR, padding: "32px" }}>
            <div className="flex items-center gap-3 mb-4">
              <span className="inline-block text-xs font-bold uppercase tracking-widest px-2 py-1 rounded" style={{ background: "rgba(255,255,255,0.2)", color: "#fff" }}>Ministry</span>
              {entity.block && (
                <span className="inline-block text-xs font-bold uppercase tracking-widest px-2 py-1 rounded" style={{ background: "rgba(255,255,255,0.15)", color: "#fff" }}>
                  📍 {entity.block}
                </span>
              )}
            </div>
            ...
            <div className="space-y-3 text-sm">
              {entity.block && (
                <div className="flex gap-3">
                  <span className="text-lg">🏢</span>
                  <div>
                    <p className="font-semibold">Alausa Block</p>
                    <p className="text-gray-500">{entity.block}</p>
                  </div>
                </div>
              )}
              ...
              {entity.phone ? (
                ...
                <div className="flex gap-3"><span className="text-lg">📞</span><div><p className="font-semibold">Phone</p><p className="text-gray-400 italic">Not available — call Alausa Secretariat switchboard</p></div></div>
              )}
              {entity.email ? (
          ...
          <div className="rounded-xl p-6" style={{ background: "#fff", border: "2px solid #0D0D0D" }}>
            ...
            {hasEmptyOfficers && (
              <div className="mb-3 p-3 rounded-lg bg-amber-50 border border-amber-200">
                <p className="text-xs text-amber-700">
                  ⚠️ Some officer positions are pending verification. Call the ministry to confirm current leadership.
                </p>
              </div>
            )}
            <div className="space-y-3">
              ...
              {entity.officers.filter(o => o.name).length === 0 && (
                <p className="text-sm text-gray-400 italic p-3">Officer information pending verification. Please call the ministry office.</p>
              )}
            </div>
          </div>
        </div>
